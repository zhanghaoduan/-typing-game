...
router.get('/', authenticate, (req, res) => {
    const myUnits = db.prepare(`
        SELECT id, name, words, phrases, sentences, is_public, created_at, updated_at
        FROM units WHERE user_id = ? ORDER BY updated_at DESC
    `).all(req.user.id);

    const publicUnits = db.prepare(`
        SELECT u.id, u.name, u.words, u.phrases, u.sentences, u.is_public, u.created_at, u.updated_at, users.username as author
...
router.post('/', authenticate, (req, res) => {
    const { name, words, phrases, sentences } = req.body;
    ...
    const result = db.prepare(`
        INSERT INTO units (user_id, name, words, phrases, sentences)
        VALUES (?, ?, ?, ?, ?)
    `).run(
        req.user.id,
        name,
...
router.put('/:id', authenticate, (req, res) => {
    ...
    db.prepare(`
        UPDATE units SET name = ?, words = ?, phrases = ?, sentences = ?, updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
    `).run(
        name || unit.name,
...
router.get('/admin/all', authenticate, requireAdmin, (req, res) => {
    const units = db.prepare(`
        SELECT u.id, u.name, u.words, u.phrases, u.sentences, u.is_public, u.created_at, u.updated_at,
